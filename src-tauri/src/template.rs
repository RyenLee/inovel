...
use crate::models::{
    WritingTemplate, UserTemplate, CreateUserTemplateParams, UpdateUserTemplateParams,
};
...
fn get_builtin_templates_path(app: &AppHandle) -> PathBuf {
    ...
    if let Ok(resource_dir) = app.path().resource_dir() {
        let template_path = resource_dir.join("builtin_templates.json");
    ...
    // 回退到 src-tauri 目录
    PathBuf::from("src-tauri/builtin_templates.json")
}
...
pub async fn get_builtin_templates(app_handle: AppHandle) -> Result<Vec<WritingTemplate>, String> {
    ...
    let content = fs::read_to_string(&path)
        .map_err(|e| format!("读取内置模板文件失败: {}", e))?;
    
    let templates: Vec<WritingTemplate> = serde_json::from_str(&content)
        .map_err(|e| format!("解析内置模板失败: {}", e))?;
...
pub async fn get_user_templates(
    ...
    let templates: Vec<UserTemplate> = stmt.query_map([project_id], |row| {
        Ok(UserTemplate {
            id: row.get(0)?,
...
pub async fn update_user_template(
    ...
    // 返回更新后的模板
    let template: UserTemplate = conn.query_row(
        "SELECT id, project_id, name, description, category, content, created_at, updated_at 
