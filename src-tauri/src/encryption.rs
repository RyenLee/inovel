...
pub fn encrypt_project_inner(
    ...
    for (index, file_path) in files_to_encrypt.iter().enumerate() {
        ...
        if file_path.file_name().map_or(false, |name| name == VAULT_FILENAME) {
            continue;
        }
    ...
    let vault_json = serde_json::to_string_pretty(&vault)
        .map_err(|e| format!("序列化 vault 失败: {}", e))?;
    ...
    fs::write(&vault_path, vault_data)
        .map_err(|e| format!("写入 vault 文件失败: {}", e))?;
...
pub fn decrypt_project_inner(
    ...
    let vault_data = fs::read(&vault_path)
        .map_err(|e| format!("读取 vault 文件失败: {}", e))?;
...
pub fn change_password_inner(
    ...
    let re_encrypted_vault = Aes256Gcm::encrypt(&new_cipher, new_vault_nonce_slice, vault_json.as_ref())
    ...
    fs::write(&vault_path, new_vault_data)
        .map_err(|e| format!("写入新 vault 文件失败: {}", e))?;
...
fn collect_files(dir: &Path, files: &mut Vec<PathBuf>) -> Result<(), String> {
    ...
    for entry in entries {
        ...
        if path.file_name().map_or(false, |name| name == VAULT_FILENAME) {
            continue;
        }
...
pub fn is_project_encrypted(project_path: &Path) -> bool {
    ...
    content.and_then(|c| serde_json::from_str::<serde_json::Value>(&c).ok())
...
#[tauri::command]
pub async fn reencrypt_project(
    project_path: String,
    password: String,
) -> Result<(), String> {
    let path = PathBuf::from(&project_path);
...
/// Tauri 命令：检查项目是否已加密
#[tauri::command]
pub async fn is_project_encrypted_command(
    project_path: String,
) -> Result<bool, String> {
    let path = PathBuf::from(&project_path);
