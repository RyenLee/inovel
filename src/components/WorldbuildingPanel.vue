  ...
  <div class="h-full flex flex-col bg-gray-50 dark:bg-gray-900">
    ...
    <div class="flex items-center justify-between px-4 py-3 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
      <div class="flex items-center gap-2">
        ...
        <span class="text-sm text-gray-500 dark:text-gray-400">({{ store.characters.length }}人物 / {{ store.locations.length }}地点 / {{ store.organizations.length }}组织)</span>
      </div>
      <n-tabs v-model:value="activeTab" type="segment" size="small" class="w-auto [&_.n-tab-pane]:px-2">
          ...
          >
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1 min-w-0">
                ...
                <!-- Description Preview -->
                <p v-if="character.personality" class="text-sm text-gray-500 dark:text-gray-400 mt-2 line-clamp-2 leading-relaxed">
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p v-if="location.description" class="text-sm text-gray-500 dark:text-gray-400 mt-2 line-clamp-2 leading-relaxed">
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p v-if="org.description" class="text-sm text-gray-500 dark:text-gray-400 mt-2 line-clamp-2 leading-relaxed">
                  {{ org.description }}
                </p>
              </div>
            </div>
      ...
      <n-drawer-content :title="editingCharacter ? '编辑人物' : '新建人物'" closable>
        <n-form label-placement="top">
          ...
          <n-form-item label="年龄">
            <n-input-number v-model:value="characterForm.age" :min="0" :max="1000" placeholder="输入年龄" style="width: 100%" />
          </n-form-item>
      ...
      <n-drawer-content :title="editingLocation ? '编辑地点' : '新建地点'" closable>
        <n-form label-placement="top">
          ...
          <n-form-item label="特色">
            <n-input v-model:value="locationForm.notable_features" type="textarea" placeholder="描述地点的特色建筑或景观" :rows="3" />
          </n-form-item>
      ...
      <n-drawer-content :title="editingOrganization ? '编辑组织' : '新建组织'" closable>
        <n-form label-placement="top">
          ...
          <n-form-item label="成员数量">
            <n-input-number v-model:value="organizationForm.member_count" :min="0" placeholder="成员数量" style="width: 100%" />
          </n-form-item>
      ...
      <n-drawer-content :title="viewingCharacter?.name || '人物详情'" closable>
        <div v-if="viewingCharacter" class="space-y-6">
          ...
          <div class="flex flex-col items-center text-center pb-4 border-b border-gray-100 dark:border-gray-700">
            ...
            <div class="flex items-center gap-2">
              <n-tag v-if="viewingCharacter.gender" :type="viewingCharacter.gender === 'male' ? 'info' : 'warning'" size="small">
          ...
          <div v-if="viewingCharacter.background">
            ...
            <p class="text-gray-700 dark:text-gray-200 leading-relaxed whitespace-pre-wrap pl-3.5">{{ viewingCharacter.background }}</p>
          </div>
...
/* n-tab-pane 选项卡内容区域间距 */
:deep(.n-tab-pane) {
  padding: var(--tab-pane-padding);
}

/* 仅在人物/地点/组织卡片之间添加垂直间距 */
:deep(.n-tab-pane > div[class*="space-y-"]) > * + * {
  margin-top: var(--tab-item-gap);
}
